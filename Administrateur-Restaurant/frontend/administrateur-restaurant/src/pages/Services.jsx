...
function Field({ label, icon: Icon, value, onChange, type = 'text', suffix, placeholder }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <label style={{ fontSize: 10, fontWeight: 900, color: focused ? GOLD : GOLD_DK, textTransform: 'uppercase', letterSpacing: '0.15em', transition: 'color 0.15s' }}>
        {label}
      </label>
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
        {Icon && (
          <Icon size={13} strokeWidth={2.5} color={focused ? GOLD : '#bbb'}
        ...
        {suffix && (
          <span style={{
            ...
            fontSize: 11, fontWeight: 800, color: focused ? GOLD_DK : '#bbb',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <button
        ...
        style={{
          ...
          background: valid && !saving ? DARK : '#e8e0d8',
          border: 'none',
          color: valid && !saving ? '#fff' : '#bbb',
          fontSize: 13, fontWeight: 800,
    ...
    >
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          ...
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 9px', background: '#f5f0eb', fontSize: 11, fontWeight: 700, color: DARK }}>
            <Users size={10} strokeWidth={2.5} color={DARK} />
            {svc.capacity} pers. max
          </span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 9px', background: '#f5f0eb', fontSize: 11, fontWeight: 700, color: DARK }}>
            <Clock size={10} strokeWidth={2.5} color={DARK} />
            {svc.duration} min
          </span>
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', borderLeft: '1px solid #e8e0d8' }}>
        <button onClick={() => onEdit(svc)}
          ...
          onMouseLeave={e => { e.currentTarget.style.background = isEditing ? '#fdf6ec' : 'none'; e.currentTarget.style.color = isEditing ? GOLD : '#aaa' }}
        >
          <Pencil size={13} strokeWidth={2.5} />
        </button>
        <button onClick={() => onDelete(svc)}
          style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 14px', background: 'none', border: 'none', color: '#ccc', cursor: 'pointer', transition: 'all 0.15s' }}
        ...
        <FadeUp delay={20}>
          <div className="svc-layout">
            ...
            <div>
              ...
              {services.length === 0 ? (
                <div style={{ padding: '56px 0', textAlign: 'center', border: '2px dashed #e8e0d8' }}>
                  <Utensils size={36} color="rgba(43,33,24,0.1)" strokeWidth={1.5} style={{ display: 'block', margin: '0 auto 14px' }} />
                  <p style={{ margin: 0, fontSize: 14, fontWeight: 900, color: 'rgba(43,33,24,0.2)' }}>Aucun service configuré</p>
                  <p style={{ margin: '6px 0 0', fontSize: 12, fontWeight: 700, color: 'rgba(43,33,24,0.15)' }}>Ajoutez votre premier service à gauche</p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {services.map(svc => (
