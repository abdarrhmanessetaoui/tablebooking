...
function StatBlock({ icon: Icon, value, label, accent, bg, delay = 0, total = 0 }) {
  ...
  return (
    <div style={{
      ...
      <span style={{
        fontSize: 'clamp(32px,4vw,52px)', fontWeight: 900, color: DARK,
      ...
      {pct !== null && (
        <>
          ...
          <span style={{ fontSize: 9, fontWeight: 900, color: accent, opacity: 0.75 }}>{pct}%</span>
        </>
      )}
    </div>
  )
