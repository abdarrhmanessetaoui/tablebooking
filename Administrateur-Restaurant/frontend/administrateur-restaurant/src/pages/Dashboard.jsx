...
import {
  CalendarDays, CalendarCheck, CheckCircle2, Clock, XCircle,
  ClipboardList, Users, TrendingUp, MapPin, Mail, ChevronRight, Sunrise,
} from 'lucide-react'

const B = {
  dark: '#3D1F0D', mid: '#7C4A20', warm: '#C48A52',
  tint: '#FBF6F1', border: '#EDE3DA',
}
...
function FadeUp({ children, delay = 0, style = {} }) {
  ...
  return (
    <div style={{ opacity: on ? 1 : 0, transform: on ? 'translateY(0)' : 'translateY(14px)', transition: 'opacity 0.45s ease, transform 0.45s ease', ...style }}>
      {children}
    </div>
  )
}
...
function Card({ children, onClick, style = {} }) {
  ...
  return (
    <div
      ...
      style={{
        background: '#fff', border: `1.5px solid ${hov ? B.border : '#F0EBE3'}`,
        borderRadius: 18, padding: 24, cursor: onClick ? 'pointer' : 'default',
        transition: 'box-shadow 0.2s, border-color 0.2s, transform 0.2s',
        boxShadow: hov ? '0 8px 28px rgba(61,31,13,0.10)' : '0 1px 4px rgba(0,0,0,0.04)',
        transform: hov && onClick ? 'translateY(-2px)' : 'none', ...style,
      }}
    >
      {children}
    </div>
    ...
    <Card onClick={onClick}>
      ...
      <div>
        ...
        <div style={{ height: 7, background: '#F3F4F6', borderRadius: 99, overflow: 'hidden' }}>
          <div style={{ height: '100%', borderRadius: 99, background: `linear-gradient(90deg,${B.dark} 0%,${B.warm} 100%)`, width: `${bar}%`, transition: 'width 1.1s cubic-bezier(0.34,1.4,0.64,1)' }} />
        </div>
  ...
  return (
    <FadeUp delay={delay} style={{ height: '100%' }}>
      <Card onClick={onClick} style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div>
          <p style={{ margin: 0, fontSize: 40, fontWeight: 900, color: '#111827', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{n}</p>
...
function QuickNav({ tomorrow, onCalendar, onReservations }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, height: '100%' }}>
      <div onClick={onCalendar} onMouseEnter={() => setH1(true)} onMouseLeave={() => setH1(false)}
        style={{ background: '#fff', border: `1.5px solid ${h1 ? B.border : '#F0EBE3'}`, borderRadius: 16, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', boxShadow: h1 ? '0 6px 20px rgba(61,31,13,0.09)' : '0 1px 4px rgba(0,0,0,0.04)', transition: 'all 0.18s ease', transform: h1 ? 'translateY(-1px)' : 'none' }}>
        ...
        <ChevronRight size={16} color={h1 ? B.mid : '#D1D5DB'} />
      </div>

      <div onClick={onReservations} onMouseEnter={() => setH2(true)} onMouseLeave={() => setH2(false)}
        style={{ background: h2 ? B.mid : B.dark, borderRadius: 16, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', boxShadow: h2 ? `0 8px 24px ${B.dark}50` : `0 4px 12px ${B.dark}30`, transition: 'all 0.18s ease', transform: h2 ? 'translateY(-1px)' : 'none' }}>
      ...
      <Card style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px' }}>
        ...
        <span style={{ fontSize: 28, fontWeight: 900, color: '#111827', fontVariantNumeric: 'tabular-nums' }}>{tomorrow}</span>
      </Card>
    </div>
  )
}
  ...
  return (
    <Card style={{ marginTop: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <p style={{ margin: 0, fontSize: 15, fontWeight: 900, color: '#111827' }}>Créneaux disponibles</p>
          <p style={{ margin: '3px 0 0', fontSize: 12, color: '#9CA3AF' }}>{active.length} créneau(x) actif(s)</p>
        </div>
        <button onClick={handleSave} disabled={saving || active.length === 0}
          style={{ padding: '8px 16px', background: saving ? '#D1D5DB' : B.dark, color: '#fff', border: 'none', borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>
          {saving ? 'Enregistrement...' : 'Enregistrer'}
        </button>
      </div>
      {success && <p style={{ fontSize: 12, color: '#059669', marginBottom: 12 }}>✓ Créneaux enregistrés</p>}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {allSlots.map(slot => {
          const isActive = active.includes(slot)
          return (
            <button key={slot} onClick={() => toggleSlot(slot)}
              style={{ padding: '8px 14px', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer', border: `1.5px solid ${isActive ? B.warm : '#E5E7EB'}`, background: isActive ? B.tint : '#fff', color: isActive ? B.dark : '#9CA3AF', transition: 'all 0.15s ease' }}>
              {slot}
            </button>
          )
        })}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAF8F5', fontFamily: 'system-ui,-apple-system,sans-serif' }}>
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: 'clamp(20px,4vw,36px)' }}>
        ...
        {/* Quick nav */}
        <FadeUp delay={270} style={{ marginBottom: 20 }}>
          <QuickNav
