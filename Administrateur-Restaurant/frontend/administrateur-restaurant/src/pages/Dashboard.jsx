...
/* ─── Btn — pixel-identical to BlockedDates ─── */
...
/* ─── Stat number block ─── */
function StatNum({ value, accent, label, icon: Icon, delay = 0, total = 0 }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <span style={{ fontSize: 9, fontWeight: 900, color: accent, textTransform: 'uppercase', letterSpacing: '0.18em' }}>
          {label}
        </span>
      </div>
      <span style={{
        fontSize: 'clamp(26px,3.5vw,42px)', fontWeight: 900, color: DARK,
        letterSpacing: '-1.5px', lineHeight: 1, fontVariantNumeric: 'tabular-nums',
      }}>{n}</span>
      {pct !== null && (
        <>
          <div style={{ height: 3, background: BORDER, overflow: 'hidden', marginTop: 2 }}>
...
/* ─── Tiny inline status badge — dot + abbreviated word ─── */
...
function ReservationsBlock({ reservations, onViewAll }) {
  ...
  const COLS = [
    ...
    { key: 'service',    label: 'Service',  flex: 1.1 },
  ...
  return (
    <div>
      <div className="res-desktop" style={{ overflowX: 'auto' }}>
        <div style={{ minWidth: 420 }}>
          {/* Header bar — same dark/gold as BlockedDateList */}
          <div style={{ display: 'grid', gridTemplateColumns: tpl, gap: 12, padding: '10px 20px', background: DARK }}>
            {COLS.map(c => (
              <span key={c.key} style={{ fontSize: 9, fontWeight: 900, color: GOLD, textTransform: 'uppercase', letterSpacing: '0.18em' }}>
                {c.label}
              </span>
            ))}
          </div>
          {reservations.slice(0, 8).map((r, i) => (
            <div key={r.id ?? i} style={{
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 900, color: DARK, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.name}</p>
                {r.phone && <p style={{ margin: '1px 0 0', fontSize: 10, fontWeight: 700, color: MUTED }}>{r.phone}</p>}
              </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ background: WHITE, border: `1px solid ${BORDER}` }}>
        {/* Dark header bar with gold label + total count badge — same as BlockedDates */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px', background: DARK }}>
          <span style={{ fontSize: 9, fontWeight: 900, color: GOLD, textTransform: 'uppercase', letterSpacing: '0.2em' }}>Vue d'ensemble</span>
          <span style={{
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            minWidth: 28, height: 28, padding: '0 9px',
            background: GOLD, color: DARK, fontSize: 13, fontWeight: 900,
          }}>{hero}</span>
        </div>
        {/* 3 stat numbers in a row, separated by thin borders */}
        <div className="stats-grid">
          <StatNum icon={CheckCircle} value={c} label="Confirmées" accent={GREEN} delay={50}  total={total} />
          <div className="stat-divider" />
          <StatNum icon={Clock}       value={p} label="En attente" accent={AMBER} delay={80}  total={total} />
          <div className="stat-divider" />
          <StatNum icon={XCircle}     value={a} label="Annulées"   accent={RED}   delay={110} total={total} />
        </div>
      </div>
      ...
      <div style={{ background: WHITE, border: `1px solid ${BORDER}` }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px', background: DARK }}>
          <span style={{ fontSize: 9, fontWeight: 900, color: GOLD, textTransform: 'uppercase', letterSpacing: '0.2em' }}>Prochaines réservations</span>
          <button onClick={onViewAll} style={{
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        /* Stats row — 3 cols separated by thin dividers */
        .stats-grid {
          display: grid;
          grid-template-columns: 1fr 1px 1fr 1px 1fr;
          align-items: start;
        }
        .stats-grid > div:not(.stat-divider) { padding: 20px 24px; }
        .stat-divider { background: ${BORDER}; align-self: stretch; }
        @media (max-width: 560px) {
          .stats-grid { grid-template-columns: 1fr; }
          .stats-grid > div:not(.stat-divider) { padding: 16px 20px; border-bottom: 1px solid ${BORDER}; }
          .stats-grid > div:not(.stat-divider):last-child { border-bottom: none; }
