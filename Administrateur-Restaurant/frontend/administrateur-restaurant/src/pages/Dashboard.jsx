...
function Badge({ status }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 10, fontWeight: 900, color: s.color, letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>
...
function ResCardMobile({ r, i, onRowClick }) {
  const [hov, setHov] = useState(false)
  return (
    <div onClick={() => onRowClick(r)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 900, color: DARK, display: 'flex', alignItems: 'center', gap: 4 }}>
          <Clock size={11} strokeWidth={2.5} color={DARK} />{r.start_time}
        </span>
        <span style={{ fontSize: 11, fontWeight: 900, color: DARK, display: 'flex', alignItems: 'center', gap: 4 }}>
          <Users size={11} strokeWidth={2.5} color={DARK} />{r.guests} personnes
        </span>
        {r.service && (
          <span style={{ fontSize: 11, fontWeight: 700, color: DARK, display: 'flex', alignItems: 'center', gap: 4 }}>
            <MapPin size={11} strokeWidth={2.5} color={DARK} />{r.service}
          </span>
        )}
      </div>
    </div>
  ...
  if (!reservations?.length) return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 16px', textAlign: 'center' }}>
        <CalendarDays size={36} color={DARK} style={{ marginBottom: 14 }} />
        <p style={{ margin: 0, fontSize: 14, fontWeight: 900, color: DARK }}>Aucune réservation</p>
        <p style={{ margin: '5px 0 0', fontSize: 11, fontWeight: 700, color: DARK }}>Les réservations apparaîtront ici</p>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'grid', gridTemplateColumns: tpl, gap: 10, padding: '11px 16px', background: DARK }}>
        {COLS.map(c => (
          <span key={c.key} style={{ fontSize: 9, fontWeight: 900, color: GOLD, textTransform: 'uppercase', letterSpacing: '0.18em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.label}</span>
        ))}
      </div>
  ...
  return (
    <div onClick={() => onRowClick(r)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, fontWeight: 900, color: DARK }}>
        <Users size={11} strokeWidth={2.5} color={DARK} />{r.guests}
      </span>
      <span style={{ fontSize: 12, fontWeight: 700, color: DARK, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.service || '—'}</span>
      <Badge status={r.status} />
              ...
              {reservations?.length
                ...
                : (
                  <div style={{ padding: '40px 16px', textAlign: 'center' }}>
                    <CalendarDays size={32} color={DARK} style={{ display: 'block', margin: '0 auto 12px' }} />
                    <p style={{ margin: 0, fontSize: 13, fontWeight: 900, color: DARK }}>Aucune réservation</p>
                    <p style={{ margin: '5px 0 0', fontSize: 11, fontWeight: 700, color: DARK }}>{periodLabel}</p>
                  </div>
                )
              }
...
export default function Dashboard() {
  ...
  function handleRowClick(r) {
    ...
    navigate('/reservations', { state: { openId: r.id, filterDate: monthDate } })
  }
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .db-tabs { display: flex; border-bottom: 2px solid ${DARK}; margin-bottom: 8px; overflow-x: auto; scrollbar-width: none; background: ${WHITE}; }
        ...
        .tab-pill { display: inline-flex; align-items: center; justify-content: center; min-width: 17px; height: 17px; padding: 0 4px; background: ${AMBER_BG}; color: ${AMBER}; font-size: 9px; font-weight: 900; }
      `}</style>
      <div style={{ minHeight: '100vh', background: CREAM, fontFamily: "'Plus Jakarta Sans','DM Sans',system-ui,sans-serif", padding: 'clamp(14px,3vw,40px) clamp(12px,4vw,36px)', width: '100%', overflowX: 'hidden', boxSizing: 'border-box' }}>
        ...
        <FadeUp delay={0}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 8, flexWrap: 'wrap' }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              <h1 style={{ margin: 0, fontSize: 'clamp(20px,5vw,36px)', fontWeight: 900, color: DARK, letterSpacing: '-1.5px', lineHeight: 1 }}>Tableau de bord</h1>
              <p className="page-subtitle" style={{ margin: '6px 0 0', fontSize: 12, fontWeight: 700, color: GOLD_DK }}><LiveClock /></p>
            </div>
            <div style={{ display: 'flex', gap: 3, flexShrink: 0 }}>
              <Btn icon={FileDown} primary onClick={handleExport} disabled={exporting}>{exporting ? 'Export…' : 'Exporter PDF'}</Btn>
            </div>
          </div>
        </FadeUp>
        <FadeUp delay={10}><div style={{ height: 2, background: DARK, margin: '16px 0 20px' }} /></FadeUp>
        <FadeUp delay={20}>
        ...
        <FadeUp delay={30} key={tab}>
          <TabPanel tab={tab} stats={stats} reservations={active?.res ?? []} tabLabel={active?.label ?? ''} tabDate={active?.date ?? null}
