...
export default function useTimeSlots({ form, setForm, services, allOH, workingDates }) {
  ...
  const openDaysLabel = useMemo(() => {
    ...
    return (selectedSvc.available_days ?? [0,1,2,3,4,5,6])
      .filter(d => workingDates[d] !== false).map(d => days[d]).join(' · ')
  }, [selectedSvc, workingDates])

  // Reset start_time if no longer valid
  useEffect(() => {
