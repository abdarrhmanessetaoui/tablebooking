...
function Badge({ status }) {
  ...
  const styles = {
    Confirmed: { border: '#16a34a', text: '#2d6a2d', dot: '#16a34a' },
    Pending:   { border: '#c8a97e', text: '#a8834e', dot: '#c8a97e' },
    Cancelled: { border: '#dc2626', text: '#DC2626', dot: '#dc2626' },
  }
  const s = styles[status] || styles.Pending

  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'3px 10px', background:'#fff', border:`4px solid ${s.border}`, fontSize:9, fontWeight:900, color:s.text, letterSpacing:'0.15em', textTransform:'uppercase', whiteSpace:'nowrap' }}>
...
function ResRow({ r, last }) {
  const { t } = useTranslation()
  const styles = {
    Confirmed: { dot: '#16a34a' },
    Pending:   { dot: '#c8a97e' },
    Cancelled: { dot: '#dc2626' },
  }
...
function CompactCard({ r }) {
  const styles = {
    Confirmed: { dot: '#16a34a', text: '#2d6a2d' },
    Pending:   { dot: '#c8a97e', text: '#a8834e' },
    Cancelled: { dot: '#dc2626', text: '#DC2626' },
  }
  const s = styles[r.status] || styles.Pending
  return (
    <div style={{ borderLeft:`3px solid ${s.dot}`, background:'#fff', padding:'5px 8px', overflow:'hidden' }}>
      ...
      <div style={{ padding:'20px 24px', background:DARK, display:'flex', flexWrap:'wrap', alignItems:'flex-start', justifyContent:'space-between', gap:16 }}>
        <div>
          ...
          <span style={{ fontSize:13, fontWeight:800, color:GOLD }}>
            {reservations.length} {t(reservations.length === 1 ? 'reservation' : 'reservation_plural')}
          </span>
        </div>
        {reservations.length > 0 && (
          <div style={{ display:'flex', gap:20 }}>
            {confirmed>0 && <div style={{ textAlign:'center' }}><div style={{ fontSize:22, fontWeight:900, color:'#fff', lineHeight:1, letterSpacing:'-1px' }}>{confirmed}</div><div style={{ fontSize:9, fontWeight:800, color:'rgba(200,169,126,0.7)', textTransform:'uppercase', letterSpacing:'0.1em', marginTop:3 }}>{t('status_confirmed')}</div></div>}
            {pending>0   && <div style={{ textAlign:'center' }}><div style={{ fontSize:22, fontWeight:900, color:GOLD, lineHeight:1, letterSpacing:'-1px' }}>{pending}</div><div style={{ fontSize:9, fontWeight:800, color:'rgba(200,169,126,0.7)', textTransform:'uppercase', letterSpacing:'0.1em', marginTop:3 }}>{t('attente')}</div></div>}
            {cancelled>0 && <div style={{ textAlign:'center' }}><div style={{ fontSize:22, fontWeight:900, color:'rgba(255,255,255,0.3)', lineHeight:1, letterSpacing:'-1px' }}>{cancelled}</div><div style={{ fontSize:9, fontWeight:800, color:'rgba(200,169,126,0.7)', textTransform:'uppercase', letterSpacing:'0.1em', marginTop:3 }}>{t('ann')}</div></div>}
          </div>
        )}
      </div>
        ...
        <div style={{ border:`4px solid ${DARK}` }}>
          <div style={{ padding:'14px 20px', background:DARK, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
            ...
            <span style={{ fontSize:12, fontWeight:800, color:GOLD }}>
              {getByDate(weekDays[activeDay]).length} {t(getByDate(weekDays[activeDay]).length === 1 ? 'reservation' : 'reservation_plural')}
            </span>
          </div>
          {(() => {
          ...
          return (
            <div key={i} style={{ border:`${isToday?2:1}px solid ${isToday?GOLD:'rgba(43,33,24,0.12)'}` }}>
              <div style={{ padding:'12px 10px', background: isToday ? GOLD : DARK }}>
                ...
                {res.length > 0 && <div style={{ marginTop:4, fontSize:10, fontWeight:800, color: isToday ? DARK : GOLD }}>{res.length} {t(res.length === 1 ? 'reservation' : 'reservation_plural')}</div>}
              </div>
  ...
  return (
    <div style={{ border:`4px solid ${DARK}` }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', background:DARK }}>
        {daysShort.map(d => (
          <div key={d} style={{ padding:'10px 4px', textAlign:'center', fontSize:9, fontWeight:900, color:GOLD, letterSpacing:'0.15em', textTransform:'uppercase' }}>{d}</div>
        ))}
      </div>
          ...
          return (
            <div key={i}
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
                ...
                {res.length > 2 && <span style={{ fontSize:10, fontWeight:900, color:GOLD, paddingLeft:4 }}>+{res.length-2} {t('more')}</span>}
              </div>
            </div>
          )
        })}
      ...
      {monthsFull.map((month, i) => {
        ...
        return (
          <div key={i}
            ...
            <div style={{ fontSize:12, fontWeight:700, color:GOLD, marginBottom: res.length>0?14:0 }}>
              {t(res.length === 1 ? 'reservation' : 'reservation_plural')}
            </div>
            {res.length > 0 && (
              <div style={{ borderTop:`4px solid ${DARK}`, paddingTop:12, display:'flex', flexDirection:'column', gap:5 }}>
                {confirmed>0 && <div style={{ display:'flex', justifyContent:'space-between' }}><span style={{ fontSize:11, fontWeight:700, color:'#2d6a2d' }}>{t('status_confirmed')}</span><span style={{ fontSize:15, fontWeight:900, color:'#16a34a' }}>{confirmed}</span></div>}
                {pending>0   && <div style={{ display:'flex', justifyContent:'space-between' }}><span style={{ fontSize:11, fontWeight:700, color:DARK }}>{t('attente')}</span><span style={{ fontSize:15, fontWeight:900, color:GOLD }}>{pending}</span></div>}
                {cancelled>0 && <div style={{ display:'flex', justifyContent:'space-between' }}><span style={{ fontSize:11, fontWeight:700, color:'#DC2626' }}>{t('ann')}</span><span style={{ fontSize:15, fontWeight:900, color:'#dc2626' }}>{cancelled}</span></div>}
              </div>
            )}
          </div>
        )
