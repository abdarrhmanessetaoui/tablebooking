...
import { LayoutGrid, Clock, CalendarDays, Utensils, Users } from 'lucide-react'
...
function getDur(services, name) {
  ...
  // Convert minutes to decimal hours — minimum 15 min
...
function getPos(startT, endT, hS, hE) {
  ...
  const l = ((s - hS) / tH) * 100
  const w = ((e - s)  / tH) * 100
  // No artificial minimum — let real duration drive width
  return {
    l:  Math.max(0, l),
    w:  Math.min(w, 100 - Math.max(0, l)),
    wn: w,
  }
}
...
function Block({ res, laneCount, laneIdx, hS, hE, services }) {
  ...
  // Adaptive display based on real % width
  ...
  return (
    ...
    >
      ...
      {/* First name */}
      {isFirst && (
        <span style={{ fontSize: 9, fontWeight: 900, color: sc.text, overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }}>
...
function Row({ row, isLast, hS, hE, hours, totalH, services, isToday }) {
  ...
  const rowH      = Math.max(52, laneCount * 44)

  return (
    <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr', borderBottom: isLast ? 'none' : `1px solid ${BORDER}`, minHeight: rowH }}>
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans','DM Sans',system-ui,sans-serif" }}>
      ...
      <div className="tl-wrap">
        <div className="tl-inner" style={{ border: `2px solid ${DARK}`, overflow: 'hidden' }}>

          {/* Hour header */}
          <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr', background: DARK }}>
