...
function getOpenHoursForDate(allOH, dateISO) {
  ...
  return { hStart: Math.max(0, Math.floor(minStart)), hEnd: Math.min(24, Math.ceil(maxEnd)) }
}
...
function Tooltip({ res, endTime }) {
  ...
  return (
    <div style={{
      ...
      transform: 'translateX(-50%)', background: DARK,
      padding: '10px 14px', zIndex: 200, pointerEvents: 'none',
      boxShadow: '0 8px 24px rgba(0,0,0,0.4)', minWidth: 190, maxWidth: 260,
    }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        {[
          { Icon: Clock,    value: `${res.start_time}${endTime ? ` – ${endTime}` : ''}` },
          { Icon: Users,    value: `${res.guests} personne${res.guests > 1 ? 's' : ''}` },
          { Icon: Utensils, value: res.service || null },
        ].filter(r => r.value).map(({ Icon, value }) => (
          <div key={value} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <Icon size={10} color={GOLD} strokeWidth={2.5} />
            <span style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.8)' }}>{value}</span>
          </div>
        ))}
      </div>
      <div style={{
  ...
  return (
    <div
      ...
      style={{
        ...
        padding:       pos.widthNum <= 3 ? '0 2px' : '0 8px',
        overflow:      'hidden',
        cursor:        'default',
        transition:    'box-shadow 0.12s, filter 0.12s',
        zIndex:        hov ? 50 : laneIndex + 2,
        boxShadow:     hov ? '0 6px 20px rgba(0,0,0,0.35)' : '0 1px 4px rgba(0,0,0,0.18)',
    ...
    >
      ...
      {isWide && (
        <>
          <span style={{ fontSize: laneCount > 2 ? 9 : 11, fontWeight: 900, color: scheme.text, overflow: 'hidden', textOverflow: 'ellipsis', lineHeight: 1.2 }}>
            {res.customer_name}
          </span>
          {laneCount <= 3 && pos.widthNum > 10 && (
            <span style={{ fontSize: 9, fontWeight: 600, color: 'rgba(255,255,255,0.65)', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: 1 }}>
              {res.start_time}–{endTime} · {res.guests}p{res.service && pos.widthNum > 18 ? ` · ${res.service}` : ''}
            </span>
          )}
        </>
      )}
...
// ── Timeline row ───────────────────────────────────────────────────
function TimelineRow({ row, isLast, hStart, hEnd, hours, totalH, services, isToday }) {
  ...
  const rowH      = Math.max(58, laneCount * 46)
  ...
  return (
    <div style={{
      ...
      {/* Info */}
      <div style={{
        padding:        '10px 14px', borderRight: `2px solid rgba(43,33,24,0.12)`,
        display:        'flex', flexDirection: 'column', justifyContent: 'center',
        background:     hasRes ? '#fdfcfb' : '#fff', flexShrink: 0, gap: 6,
      }}>
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          ...
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 7px', background: locStyle.bg, fontSize: 9, fontWeight: 700, color: locStyle.color }}>
            <MapPin size={8} strokeWidth={2.5} />{row.location}
          </span>
        </div>
      </div>
      ...
      <div style={{ position: 'relative', overflow: 'visible', background: hasRes ? '#fff' : '#fafaf8', minHeight: rowH }}>
        {hours.map((h, i) => (
          <div key={`bg-${h}`} style={{ position: 'absolute', left: `${(i / totalH) * 100}%`, width: `${(1 / totalH) * 100}%`, top: 0, bottom: 0, background: i % 2 === 0 ? 'transparent' : 'rgba(43,33,24,0.018)', pointerEvents: 'none' }} />
        ))}
        {hours.map((h, i) => i > 0 && (
          <div key={`line-${h}`} style={{ position: 'absolute', left: `${(i / totalH) * 100}%`, top: 0, bottom: 0, width: 1, background: 'rgba(43,33,24,0.08)', pointerEvents: 'none' }} />
        ))}
        {hours.map((h, i) => (
          <div key={`half-${h}`} style={{ position: 'absolute', left: `${((i + 0.5) / totalH) * 100}%`, top: 0, bottom: 0, width: 1, background: 'rgba(43,33,24,0.03)', pointerEvents: 'none' }} />
        ))}
        ...
        {isToday && (() => {
          ...
          return (
            <div style={{ position: 'absolute', left: `${((dec - hStart) / totalH) * 100}%`, top: 0, bottom: 0, width: 2, background: RED, zIndex: 40, pointerEvents: 'none', boxShadow: `0 0 6px ${RED}88` }} />
          )
        })()}

        {!hasRes && (
          <span style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', fontSize: 9, fontWeight: 800, color: 'rgba(43,33,24,0.1)', letterSpacing: '0.25em', textTransform: 'uppercase', pointerEvents: 'none', userSelect: 'none' }}>
            disponible
          </span>
        )}

        {lanedRes.map(res => (
          <ResBlock key={res.id} res={res} laneCount={laneCount} laneIndex={res.lane} hStart={hStart} hEnd={hEnd} services={services} />
        ))}
      </div>
    </div>
  )
...
export default function TableTimeline({ controlledDate = null }) {
  ...
  const { hStart, hEnd } = useMemo(() => getOpenHoursForDate(allOH, date), [allOH, date])
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans','DM Sans',system-ui,sans-serif", marginTop: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 200, display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: DARK }}>
          ...
          {isToday && (
            <span style={{ fontSize: 9, fontWeight: 900, color: GOLD, letterSpacing: '0.14em', textTransform: 'uppercase', flexShrink: 0 }}>
              Aujourd'hui
            </span>
          )}
        </div>
        ...
        <input
          ...
          style={{ padding: '7px 10px', border: `2px solid ${DARK}`, fontSize: 12, fontWeight: 700, color: DARK, fontFamily: 'inherit', background: '#fff', cursor: 'pointer', outline: 'none', height: 36 }}
        />

        {!isToday && (
          <button
            onClick={() => setDate(today)}
            style={{ padding: '7px 14px', background: GOLD, border: 'none', fontSize: 11, fontWeight: 900, color: DARK, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', height: 36, flexShrink: 0 }}
      ...
      <div style={{ border: `2px solid ${DARK}`, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', background: DARK }}>
          ...
          <div style={{ position: 'relative', height: 36, overflowX: 'hidden' }}>
            {hours.map((h, i) => {
              ...
              return (
                <div key={h} style={{ position: 'absolute', left: `${(i / totalH) * 100}%`, width: `${(1 / totalH) * 100}%`, top: 0, bottom: 0, display: 'flex', alignItems: 'center', paddingLeft: 5, borderLeft: i > 0 ? '1px solid rgba(200,169,126,0.12)' : 'none', background: isNow ? 'rgba(239,68,68,0.1)' : 'transparent' }}>
        ...
        {/* Tables with no reservations this day — show once, no duplicate empty rows */}
        {!loading && timeline.length > 0 && totalRes === 0 && (
          <>
            {/* Empty message banner inside grid */}
            <div style={{ padding: '22px 24px', background: CREAM, borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ width: 36, height: 36, background: '#f0ebe4', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <CalendarDays size={17} color={GOLD_DARK} strokeWidth={2} />
              </div>
              <div>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 900, color: DARK }}>
                  Aucune réservation le {formatDateShort(date)}
                </p>
                <p style={{ margin: '3px 0 0', fontSize: 11, fontWeight: 600, color: 'rgba(43,33,24,0.4)' }}>
                  {timeline.length} table{timeline.length > 1 ? 's' : ''} disponible{timeline.length > 1 ? 's' : ''} ce jour-là
                </p>
              </div>
            </div>
            {/* Still render rows so user sees which tables exist */}
            {timeline.map((row, i) => (
              <TimelineRow key={row.table_id} row={row} isLast={i === timeline.length - 1} hStart={hStart} hEnd={hEnd} hours={hours} totalH={totalH} services={services} isToday={isToday} />
            ))}
          </>
        )}

        {/* Normal rows with reservations */}
