...
// ── Timeline config ────────────────────────────────────────────────
const HOUR_START = 10   // start at 10:00
const HOUR_END   = 24   // end at 00:00
...
function blockPosition(startTime, endTime) {
  ...
  // Default slot = 1.5h if no end_time
  const end   = toDecimal(endTime) ?? (start + 1.5)
  const left  = ((start - HOUR_START) / TOTAL_H) * 100
  const width = ((end - start) / TOTAL_H) * 100
  return {
    left:  Math.max(0, Math.min(left, 100)).toFixed(3) + '%',
    width: Math.max(1, Math.min(width, 100 - Math.max(0, parseFloat(left)))).toFixed(3) + '%',
  }
}
...
function assignLanes(reservations) {
  ...
  return sorted.map(res => {
    ...
    const end   = toDecimal(res.end_time)   ?? (start + 1.5)
    ...
    >
      ...
      {laneCount <= 2 && (
        <span style={{ fontSize: 9, fontWeight: 600, color: 'rgba(255,255,255,0.6)', marginLeft: 4, flexShrink: 0 }}>
          {res.start_time}{res.end_time ? `–${res.end_time}` : ''} · {res.guests}p
        </span>
      )}
    </div>
  )
...
function TimelineRow({ row, isLast }) {
  ...
  // Assign lanes to handle overlapping reservations
  const lanedRes  = assignLanes(row.reservations)
  const laneCount = lanedRes.length > 0
    ? Math.max(...lanedRes.map(r => r.lane)) + 1
    : 1

  // Dynamic row height based on lane count
  const rowHeight = Math.max(52, laneCount * 28)
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans','DM Sans',system-ui,sans-serif", marginTop: 24 }}>
      ...
      <div style={{ border: `2px solid ${DARK}`, overflow: 'hidden' }}>
        ...
        {!loading && timeline.length === 0 && (
          <div style={{ padding: '52px 24px', textAlign: 'center', background: '#fff' }}>
            ...
            <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: 'rgba(43,33,24,0.4)' }}>Configurez vos tables dans la section Tables.</p>
          </div>
        )}

        {/* Rows */}
        {!loading && timeline.map((row, i) => (
          <TimelineRow key={row.table_id} row={row} isLast={i === timeline.length - 1} />
        ))}
      </div>
      ...
      {!loading && timeline.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', marginTop: 8, padding: '8px 14px', background: CREAM, border: `1px solid ${BORDER}` }}>
          ...
          <span style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 600, color: 'rgba(43,33,24,0.35)', whiteSpace: 'nowrap' }}>
