...
export default function TimeSlotPicker({ value, onChange, slots }) {
  ...
  return (
    <div style={{ display:'flex', flexWrap:'wrap', gap:7, marginTop:4 }}>
      {slots.map(slot => (
        <button key={slot} onClick={() => onChange(slot)}style={{ padding:'7px 13px', background:value===slot?DARK:GOLD, border:'none', borderRadius:999, fontSize:12, fontWeight:800, color:value===slot?GOLD:DARK, cursor:'pointer' }}>{slot}</button>
      ))}
    </div>
  )
}