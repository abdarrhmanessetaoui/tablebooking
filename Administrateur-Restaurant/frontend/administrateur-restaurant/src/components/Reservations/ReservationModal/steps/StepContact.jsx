...
import { DARK, LIGHT_BROWN, STATUS_CONFIG } from '../../../../styles/reservations/tokens'
    ...
    <>
      ...
      <div>
        ...
        <textarea value={form.notes??''} onChange={e=>setForm({...form,notes:e.target.value})} rows={2}
          ...
          onFocus={e=>e.target.style.borderColor=LIGHT_BROWN} onBlur={e=>e.target.style.borderColor='#e8e0d8'} />
      </div>
      <div>
        <Label text={t('status')} />
        <div style={{ display:'flex', gap:6 }}>
          {['Confirmed','Pending','Cancelled'].map(s => (
            <button key={s} onClick={() => setForm({...form,status:s})}
              style={{ flex:1, padding:'10px 4px', background:form.status===s?DARK:'#f5f0eb', border:'none', fontSize:12, fontWeight:900, color:form.status===s?LIGHT_BROWN:'#888', cursor:'pointer' }}>
