    ...
    <>
      <div>
        ...
        <select value={form.service??''} onChange={e => setForm({...form,service:e.target.value,date:'',start_time:'',guests:''})}
          style={selectStyle} onFocus={e=>e.target.style.borderColor=LIGHT_BROWN} onBlur={e=>e.target.style.borderColor='#e8e0d8'}>
      ...
      <button onClick={onNext} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
        style={{ padding:'14px', background:hov?LIGHT_BROWN_DARK:LIGHT_BROWN, border:'none', fontSize:14, fontWeight:900, color:DARK, cursor:'pointer' }}>
        {t('next_step')}
      </button>
    </>
  )
