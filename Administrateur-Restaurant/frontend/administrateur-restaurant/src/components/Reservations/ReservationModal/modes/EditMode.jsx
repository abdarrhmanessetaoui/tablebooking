  ...
  return (
    <>
      <div style={{ background:'#faf8f5', padding:'14px 18px', display:'flex', flexDirection:'column', gap:8 }}>
        {[
          [t('date'), editing?.date],
          [t('time'), editing?.start_time],
          [t('guests'), editing?.guests]
        ].map(([l,v]) => v ? (
          ...
          {['Confirmed','Pending','Cancelled'].map(s => (
            <button 
              ...
              style={{ 
                flex:1, 
                padding:'11px 6px', 
                background:form.status===s?DARK:'#f5f0eb', 
                border:'none', 
                fontSize:12, 
                fontWeight:900, 
                color:form.status===s?GOLD:'#888', 
                cursor:'pointer' 
              }}>
              {t(s.toLowerCase())}
            </button>
      ...
      <div style={{ display:'flex', gap:8 }}>
        <button 
          onClick={onClose} 
          style={{ flex:1, padding:'12px', background:'#f5f0eb', border:'none', fontSize:13, fontWeight:800, color:DARK, cursor:'pointer' }}>
          {t('cancel')}
        </button>
